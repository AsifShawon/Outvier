...
export const uploadController = {
  async uploadUniversities(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.file) {
  ...
  async uploadPrograms(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
