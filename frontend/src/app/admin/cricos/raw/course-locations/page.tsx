...
export default function RawCourseLocationsPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await cricosApi.getRawCourseLocations({ providerCode: providerCode || undefined, page });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground">
        {total.toLocaleString()} course-location mappings found
      </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-muted/50 border-b">
            <tr>
              ...
              <th className="p-4 text-sm font-semibold">Course Code</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="p-12 text-center"><Loader2 className="animate-spin mx-auto h-6 w-6 text-muted-foreground" /></td></tr>
            ) : data.length === 0 ? (
              <tr><td colSpan={6} className="p-12 text-center text-muted-foreground">No course-location mappings found</td></tr>
            ) : data.map((item) => (
              <tr key={item._id} className="border-b text-sm hover:bg-muted/30 transition-colors">
                ...
                <td className="p-4 font-mono text-xs">{item.cricosCourseCode}</td>
                <td className="p-4 max-w-[200px] truncate">{item.locationName}</td>
                <td className="p-4">{item.locationCity ?? '—'}</td>
                <td className="p-4">{item.locationState ?? '—'}</td>
