...
export default function RawCoursesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await cricosApi.getRawCourses({ q: search, page });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Input 
            placeholder="Search name or course code..." 
            value={search}
            ...
            {loading ? (
              ...
              <tr key={item._id} className="border-b text-sm">
                ...
                <td className="p-4 text-primary font-medium">
                  {item.estimatedTotalCourseCost ? `$${item.estimatedTotalCourseCost.toLocaleString()}` : 'N/A'}
                </td>
              </tr>
            ))}
          </tbody>
